...
### With auto-bootstrap (recommended for existing projects)

```bash
./scripts/init-second-brain.sh --bootstrap /path/to/your/project
```

The `--bootstrap` flag scans your project and auto-populates the brain from existing data:
- **README.md** → mission statement + project overview doc
- **package.json / pyproject.toml / Cargo.toml** → tech stack, dependencies, build scripts
- **Git history** → active branches, recent activity, project maturity
- **ADR files** → converted to decision-record notes
- **CLAUDE.md** → principles, code style, setup commands
- **Claude memory** (`~/.claude/projects/`) → imported as knowledge notes

All auto-generated notes are marked `[BOOTSTRAPPED — REVIEW]` so you know to verify them.
