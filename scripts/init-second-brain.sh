...
for arg in "$@"; do
  case "$arg" in
    ...
    --auto) AUTO=true; BOOTSTRAP=true ;;  # --auto implies --bootstrap
    *) POSITIONAL_ARGS+=("$arg") ;;
  esac
done
set -- "${POSITIONAL_ARGS[@]}"
...
# ══════════════════════════════════════════════════════════════════════
# 1b. BOOTSTRAP (optional — auto-populate brain from project data)
# ══════════════════════════════════════════════════════════════════════
if [ "$BOOTSTRAP" = "true" ]; then
    echo ""
    echo "── Bootstrap: Scanning Project ──────────────────────────"

    if ! command -v node &>/dev/null; then
        print_error "Node.js required for --bootstrap. Skipping."
    else
        MANIFEST_FILE="$BRAIN_DIR/.bootstrap-manifest.json"
        node "$FRAMEWORK_DIR/scripts/bootstrap-scan.mjs" "$TARGET_DIR" > "$MANIFEST_FILE" 2>/dev/null
        print_success "Project scanned"

        echo ""
        echo "── Bootstrap: Populating Brain ─────────────────────────"
        node "$FRAMEWORK_DIR/scripts/bootstrap-populate.mjs" "$BRAIN_DIR" "$MANIFEST_FILE"
        print_success "Brain populated from project data"
...
# ══════════════════════════════════════════════════════════════════════
# 1c. AUTO-POPULATE (optional — LLM-generated brain content)
# ══════════════════════════════════════════════════════════════════════
if [ "$AUTO" = "true" ]; then
    echo ""
    echo "── Auto-Populate: Generating Brain Content ──────────────"

    if ! command -v node &>/dev/null; then
        print_error "Node.js required for --auto. Skipping."
    else
        # Generate the LLM auto-populate prompt
        node "$FRAMEWORK_DIR/scripts/auto-populate-prompt.mjs" "$BRAIN_DIR" "$TARGET_DIR"
        print_success "Auto-populate prompt generated at $BRAIN_DIR/hooks/.state/auto-populate-prompt.md"
        print_info "On next Claude Code session, the brain will auto-populate from this prompt."
    fi
fi
...
if [ -d "$BRAIN_DIR/hooks" ]; then
    chmod +x "$BRAIN_DIR/hooks/"*.sh "$BRAIN_DIR/hooks/"*.mjs 2>/dev/null || true
    print_success "Hook scripts made executable"
fi

# Ensure .state directory exists for session tracking
mkdir -p "$BRAIN_DIR/hooks/.state" 2>/dev/null
    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "\$CLAUDE_PROJECT_DIR/${BRAIN_FOLDER_NAME}/hooks/session-orient.sh",
            "timeout": 5000
          }
        ]
      }
...
SETTINGSEOF
    ...
    elif ! grep -q "session-orient\|brain-router" "$SETTINGS_FILE" 2>/dev/null; then
        # Merge brain hooks into existing settings
        print_info "Merging brain hooks into existing .claude/settings.json..."

        ORIENT_HOOK="{\"matcher\":\"\",\"hooks\":[{\"type\":\"command\",\"command\":\"\$CLAUDE_PROJECT_DIR/${BRAIN_FOLDER_NAME}/hooks/session-orient.sh\",\"timeout\":5000}]}"
        ...
        EVAL_HOOK="{\"matcher\":\"\",\"hooks\":[{\"type\":\"command\",\"command\":\"\$CLAUDE_PROJECT_DIR/${BRAIN_FOLDER_NAME}/hooks/session-eval-prompt.sh\",\"timeout\":15000}]}"

        MERGED=$(jq \
            --argjson orient "$ORIENT_HOOK" \
            --argjson updater "$UPDATER_HOOK" \
            --argjson eval "$EVAL_HOOK" \
            '.hooks.UserPromptSubmit = ((.hooks.UserPromptSubmit // []) + [$orient]) | .hooks.PostToolUse = ((.hooks.PostToolUse // []) + [$updater]) | .hooks.Stop = ((.hooks.Stop // []) + [$eval])' \
            "$SETTINGS_FILE")
    ...
    echo ""
    echo '  "hooks": {'
    echo '    "UserPromptSubmit": [{"matcher":"","hooks":[{"type":"command","command":"$CLAUDE_PROJECT_DIR/.brain/hooks/session-orient.sh","timeout":5000}]}],'
    ...
    echo '    "Stop": [{"matcher":"","hooks":[{"type":"command","command":"$CLAUDE_PROJECT_DIR/.brain/hooks/session-eval-prompt.sh","timeout":15000}]}]'
    echo '  }'
    echo ""
fi
